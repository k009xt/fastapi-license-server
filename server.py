...
LICENSE_DAYS = 30
GRACE_DAYS = 5
HMAC_SECRET = os.getenv("HMAC_SECRET").encode()  # Берем из .env
...
def check_license(data: CheckRequest):
 ...
 for row in rows:
     if row["client_id"] == data.client_id and row["license_key"] == data.license_key:
         ...
         if now <= expires_at:
             status_dict = {"status": "valid", "expires_at": row["expires_at"]}
         elif now <= expires_at + datetime.timedelta(days=GRACE_DAYS):
             status_dict = {"status": "grace", "expires_at": row["expires_at"]}
         else:
             status_dict = {"status": "expired", "expires_at": row["expires_at"]}
         # Добавляем HMAC-подпись
         status_json = json.dumps(status_dict, sort_keys=True).encode()
         signature = hmac.new(HMAC_SECRET, status_json, hashlib.sha256).digest()
         signature_b64 = base64.b64encode(signature).decode()
         response_dict = {**status_dict, "signature": signature_b64}
